...
import { Facility, Vendor, Staff } from '$lib/db.server';
import sanitize from '$lib/sanitize.server';
import { validate_JWT_login_admin } from '$lib/security_util.server';
import { editExistingFacilityAction, removeStaffFromPositionAtFacility, removeVendorFromRoleAtFacility } from '$lib/location_utils.server.js';
import { addVendorContractAction, getExistingContract_VendorContract, removeVendorContractAction } from '$lib/vendor_contract_utils.server.js';
...
export const actions = {
    ...
    uploadContract: async ({ cookies, request, url }) => {

        const hack = url.pathname;
        const data = await request.formData();
        const facility_id = sanitize.toInt(data.get("facility_id") as string);

        let result = await addVendorContractAction(cookies, data, url);
    ...
    removeContract: async ({ cookies, request, url }) => {

        const hack = url.pathname;
        const data = await request.formData();
        const facility_id = sanitize.toInt(data.get("facility_id") as string);

        let result = await removeVendorContractAction(cookies, data, url);
    ...
    saveFacility: async ({ cookies, request, url }) => {

		const hack = url.pathname;
        const data = await request.formData();

        const result = await editExistingFacilityAction(cookies, data, url);
